...
const EditAddress = ({ handleModal, title, desc, number, isMain }) => {
    const [recipient, setRecipient] = useState(title);
    const [address, setAddress] = useState(desc);
    ...
    return (
        <div className="fixed top-1/2 left-1/2 p-2 bg-white h-[500px] overflow-y-auto -translate-x-1/2 -translate-y-1/2">
          <Title onClick={handleModal} title='Edit address' />
          <div className='flex flex-col gap-8 w-full'>
                      <div style={{ backgroundColor: 'rgba(0, 68, 65, 0.02)'}} className='p-2 w-full flex-1 flex gap-2 flex-col'>
                              <label htmlFor="recipient" className='flex flex-col gap-1'>
                                  ...
                                  <input value={recipient} onChange={(e) => setRecipient(e.target.value)} className='bg-white border p-2 rounded-lg w-full' placeholder='Jhon Doe' type="text" />
                              </label>
                              <label htmlFor="number" className='flex flex-col gap-1'>
                                  <p className='text-sm font-bold tracking-[1.5px]'>Phone Number</p>
                                  <input value={addressNumber} onChange={(e) => setAddressNumber(e.target.value)} className='bg-white border p-2 rounded-lg w-full' placeholder='Ex: 081234567896' type="tel" />
                              </label>
                              <label htmlFor="address" className='flex flex-col gap-1'>
                                  <p className='text-sm font-bold tracking-[1.5px]'>Full address</p>
                                  <textarea value={address} onChange={e => setAddress(e.target.value)} className='bg-white border p-2 rounded-lg w-full resize-none h-[100px]' />
                              </label>
                              ...
                              <label htmlFor="addMain" className='flex gap-2 items-center'>
                                  <input checked={main} onChange={() => setMain(!main)} type="checkbox" className='h-5 w-5 accent-primary' id="addMain" />
